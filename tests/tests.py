...
class EmailTest(TestCase):
    ...
    def test_send_email(self):
        ...
        login = self.client.login(
            username="terminator", password="skynetMyLove"
        )
...
class PostViewTest(TestCase):
    ...
    def test_post_displayed_on_all_pages(self):
        ...
        self.assertEqual(
            str(response.context["paginator"].object_list[0]),
        ...
        self.assertEqual(
            str(response.context["post"]), "It's driving me crazy!"
        )
    ...
    def test_post_edit(self):
        ...
        """
        self.client.post(
            reverse("post_edit", kwargs={"username": "sarah", "post_id": 1}),
            {"text": "I ll be back!"},
            follow=True,
        )
...
class ImageTest(TestCase):
    ...
    def setUp(self):
        ...
        self.post = Post.objects.create(
            text="It's driving me crazy!", author=self.user, group=self.group
        )
        # залогинем sarah
        self.client.login(username="sarah", password="12345")
    ...
    def test_image_on_pages(self):
        ...
        )  # на всякий случай проверим новый текст
    ...
    def test_valid_image(self):
        ...
        with open(img, "rb") as fp:
            response = self.client.post(
                "/new/", {"text": "I ll be back!", "image": fp, "title": "псы"}
            )
        # проверяем редирект
        self.assertEqual(response.status_code, 302)
...
class FollowTest(TestCase):
    ...
    def setUp(self):
        ...
        self.post = Post.objects.create(
            text="It s driving me crazy!", author=self.user1
        )
    ...
    def test_post_for_following_user_only(self):
        ...
        )  # НЕ отобразилась запись testuser1
        self.assertNotContains(
            response, text="I ll be back"
        )  # testuser2 НЕ должен видеть свою запись в ленте
...
class CommentTest(TestCase):
    ...
    def test_logged_in_user_comments_only(self):
        ...
        response = self.client.post(
            reverse(
                "add_comment", kwargs={"username": "testuser1", "post_id": 1}
            ),
            {"text": "Hi, Sarah!"},
        )
        ...
        self.assertRedirects(
            response,
            "/auth/login/?next=/testuser1/1/comment",
        )
