...
class TestFollow:
    def test_follow(self):
        ...
        ), "Добавьте пользователя, автор который создал событие `user` модели `Follow`"
        assert (
            type(user_field) == fields.related.ForeignKey
        ), "Свойство `user` модели `Follow` должно быть ссылкой на другую модель `ForeignKey`"
        assert (
            user_field.related_model == get_user_model()
        ), "Свойство `user` модели `Follow` должно быть ссылкой на модель пользователя `User`"
        assert (
            user_field.remote_field.related_name == "follower"
        ...
        ), "Добавьте пользователя, автор который создал событие `author` модели `Follow`"
        assert (
        ...
        assert (
            author_field.remote_field.related_name == "following"
    ...
    def check_url(self, client, url, str_url):
        try:
            ...
            assert (
                False
            ), f"""Страница `{str_url}` работает неправильно. Ошибка: `{e}`"""
    ...
    def test_follow_not_auth(self, client, user):
        ...
        if not (
            ...
            and response.url.startswith(f"/auth/login")  # noqa
        ):
            assert (
                False
        ...
        response = self.check_url(
            client, f"/{user.username}/follow", "/<username>/follow/"
        )
        if not (
            response.status_code in (301, 302)
            and response.url.startswith(f"/auth/login")  # noqa
        ...
        response = self.check_url(
            client, f"/{user.username}/unfollow", "/<username>/unfollow/"
        )
        if not (
            response.status_code in (301, 302)
            and response.url.startswith(f"/auth/login")  # noqa
    ...
    def test_follow_auth(self, user_client, user, post):
        ...
        self.check_url(
            user_client, f"/{user_1.username}/follow", "/<username>/follow/"
        )
        assert (
        ...
        Post.objects.create(
            text="Тестовый пост 354745", author=user_1, image=image
        )

        Post.objects.create(
            text="Тестовый пост 245456", author=user_2, image=image
        )
        Post.objects.create(
            text="Тестовый пост 9789", author=user_2, image=image
        )
        Post.objects.create(
            text="Тестовый пост 4574", author=user_2, image=image
        )

        response = self.check_url(user_client, f"/follow", "/follow/")  # noqa
        assert (
            "paginator" in response.context
        ), "Проверьте, что передали переменную `paginator` в контекст страницы `/follow/`"
        assert (
            type(response.context["paginator"]) == Paginator
        ), "Проверьте, что переменная `paginator` на странице `/follow/` типа `Paginator`"
        assert (
            "page" in response.context
        ), "Проверьте, что передали переменную `page` в контекст страницы `/follow/`"
        assert (
            type(response.context["page"]) == Page
        ), "Проверьте, что переменная `page` на странице `/follow/` типа `Page`"
        assert (
            len(response.context["page"]) == 2
        ), "Проверьте, что на странице `/follow/` список статей авторов на которых подписаны"

        self.check_url(
            user_client, f"/{user_2.username}/follow", "/<username>/follow/"
        )
        assert (
        ...
        response = self.check_url(user_client, f"/follow", "/follow/")  # noqa
        assert (
            len(response.context["page"]) == 5
        ), "Проверьте, что на странице `/follow/` список статей авторов на которых подписаны"

        self.check_url(
            user_client, f"/{user_1.username}/unfollow", "/<username>/unfollow/"
        )
        assert (
            user.follower.count() == 1
        ...
        self.check_url(
            user_client, f"/{user_2.username}/unfollow", "/<username>/unfollow/"
        )
        assert (
            user.follower.count() == 0
        ), "Проверьте, что вы можете отписаться от пользователя"
